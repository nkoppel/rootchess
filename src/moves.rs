...
impl Move {
    ...
    pub fn pack(start: usize, end: usize, piece: usize) -> Self {
        Move((start as u16 & SQUARE) << 10 | (end as u16 & SQUARE) << 4 | piece as u16 & PIECE)
    }
    ...
    pub fn start(&self) -> usize {
        (self.0 >> 10 & SQUARE) as usize
    }
    pub fn end(&self) -> usize {
        (self.0 >> 4 & SQUARE) as usize
    }
...
impl Board {
    pub fn do_move(&self, mov: Move) -> Board {
        ...
        // En Passant
        if self.pawns() & 1 << start != 0 && self.takeable_empties() & 1 << end != 0 {
        ...
        // Castling (Normal)
        else if self.kings() & 1 << start != 0 && abs_diff(start, end) == 2 {
        ...
        // Double-Moving Pawns
        else if self.pawns() & 1 << start != 0 && (start as isize - end as isize).abs() == 16 {
    ...
    pub fn get_move(&self, other: &Board, c960: bool) -> Move {
        ...
        if castle < 64 {
            ...
            let start = (self.pawns() & cur_occ & diff).trailing_zeros() as usize;
            ...
            if (self.b >> start as u64) & u64x4::splat(1)
                != (other.b >> end as u64) & u64x4::splat(1)
...
fn get_test_cases() -> Vec<(bool, Board, Move, Board)> {
    vec![
        ...
        (
            ...
            vec![(
                "c1a1",
                "2kr4/pp5p/6p1/3pp3/P3n3/1PP3P1/1Q5P/2KRB3 b - - 0 27",
            )],
        ),
        (
            true,
            ...
            vec![(
                "f8g8",
                "1rnbbrkn/p1p2ppp/8/1p1B4/1P1P4/3N4/1R1P1PPP/4BKRN w G - 1 11",
            )],
        ),
    ]
    .into_iter()
    .flat_map(|(c960, fen1, moves)| {
        ...
        moves.into_iter().map(move |(m, fen2)| {
            (
                c960,
                board.clone(),
                m.parse().unwrap(),
